...
const vnpayReturn = async (req, res) => {
  try {
    ...
    for (const key of Object.keys(vnp_Params)) {
      if (key.includes(' ')) {
        console.warn(`❗ Lỗi key có dấu cách trong query: "${key}"`);
      }
    }
    ...
    console.log('🔐 [RETURN] Chữ ký tạo ra:', signed);
    ...
    if (secureHash.toLowerCase() === signed.toLowerCase()) {
      ...
      if (!order) {
        console.error('❌ Không tìm thấy đơn hàng với dh_id:', orderId);
        return res.status(404).send('❌ Không tìm thấy đơn hàng');
      }
      ...
      console.log('✅ Đơn hàng đã được đánh dấu là "paid"');
      return res.send('✅ Thanh toán thành công!');
    } else {
      console.warn('❌ Chữ ký không khớp!');
      console.warn('📩 Dữ liệu từ query:', vnp_Params);
      return res.status(400).send('❌ Chữ ký không hợp lệ!');
    }
